...
terminal.readLine().then(function(userName) {
	...
	terminal.readLine().then(function(password) {
		...
		terminal.setPrompt(">");
		terminal.print("making new connection");
				...
				if (stanza.name == "stream:features") {
					...
					if (stanza.attributes.id == "bind0") {
						...
						core.register("onInput", function(input) {
							socket.write("<message type='groupchat' to='chadhappyfuntime@conference.jabber.troubleimpact.com'><body>" + input + "</body></message>");
						});
					}
				} else if (stanza.name == "message" && stanza.attributes.type == "groupchat") {
					terminal.print("got a group chat message");
					var body;
					var delayed = false;
					...
					if (challenge.rspauth) {
						...
						terminal.print("challenge error: " + error);
					}
				}
			});
		});
